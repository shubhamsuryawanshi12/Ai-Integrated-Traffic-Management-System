...
# PCU values based on IRC:106-1990
PCU_VALUES = {
    "motorcycle": 0.5,
    "car": 1.0,
    "auto": 1.0,
...
def get_pcu(vehicle_type: str) -> float:
    """
    Get PCU value for a vehicle type.
    ...
    Returns:
        PCU value
    """
    return PCU_VALUES.get(vehicle_type.lower(), 1.0)


def compute_pcu_queue(
    vehicle_counts: dict,
) -> np.ndarray:
    """
    Compute PCU-weighted queue from vehicle counts.
    
    Args:
        vehicle_counts: Dict mapping vehicle type to count per approach
        
    Returns:
        PCU queue array
    """
    pcu_queue = np.zeros(4)  # 4 approaches
    
    for approach_idx, counts in vehicle_counts.items():
        if isinstance(counts, dict):
            for vtype, count in counts.items():
                pcu_queue[approach_idx] += get_pcu(vtype) * count
        else:
            # Assume all cars
            pcu_queue[approach_idx] = counts * 1.0
